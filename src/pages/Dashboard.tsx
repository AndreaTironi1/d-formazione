              ...
              {[1, 2, 3, 4, 5].map((p) => {
                ...
                return (
                  <div key={p} className="flex items-center gap-3">
                    <span className="text-xs text-slate-500 w-20">P{p} — {labels[p - 1]}</span>
                    <div className="flex-1 bg-slate-100 rounded-full h-2">
                      <div
                        className={`h-2 rounded-full ${colors[p - 1]} transition-all`}
                        style={{ width: corsi.length ? `${(count / corsi.length) * 100}%` : '0%' }}
                      />
                    </div>
